					...
					.all([
						(async () => {
							if (images) {
								...
								console.log('screenshot path with filename:', path.join(saveDir, courseName, 'cluster', 'screenshots', `${ sanitize(newTitle) }.png`));
						...
						(async () => {
							...
							if (markdown) {
								...
								console.log('mardkown path with filename:', path.join(saveDir, courseName, 'markdown', 'screenshots', `${ sanitize(newTitle) }.md`));
