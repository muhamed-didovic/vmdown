...
async function scrapePage(page, pageUrl, markdown, saveDir, nhm, images, videoFormat) {
	...
	if (markdown) {
		...
		await fs.writeFile(path.join(saveDir, courseName, 'cluster', 'markdown', `${ sanitize(newTitle) }.md`), nhm.translate(r.markdown), 'utf8')
	}
	...
	await createHtmlPage(page, path.join(saveDir, courseName, 'cluster', 'html'), `${ sanitize(newTitle) }`);

	await extractResources(page, path.join(saveDir, courseName, 'cluster', 'resources'), sanitize(newTitle), nhm);
